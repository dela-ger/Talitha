...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // Create description from excerpt or first paragraph of content
    let description = 'A story of hope from Talitha';
    if (story.excerpt && typeof story.excerpt === 'string') {
      description = story.excerpt.substring(0, 160) + (story.excerpt.length > 160 ? '...' : '');
...
export default async function StoryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // Safely handle content splitting with validation
    let paragraphs: string[] = [];
    if (story.content) {
      try {
        // If content is stored as JSON, parse it first
        let contentText = story.content;
        if (typeof contentText === 'object') {
          contentText = JSON.stringify(contentText);
        }
        
        // Ensure content is a string and split safely
        ...
        console.error('Error processing story content:', contentError);
        paragraphs = ['Error loading story content'];
      }
    } else {
        ...
        <div className="max-w-5xl mx-auto px-4 py-12 sm:px-6 lg:px-8">
          ...
          <article className="bg-white rounded-xl shadow-xl overflow-hidden border border-lime-100 transition-all duration-300 hover:shadow-2xl">
            ...
            <div className="relative h-96 w-full">
              <Image
                src={story.image_url || '/placeholder-image.jpg'}
